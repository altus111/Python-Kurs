...
class Fraction:
    ...
    def parse_number(value):
        # Wenn schon eine Zahl ist (int oder float), direkt zurückgeben
        if isinstance(value, (int, float)):
            return value
        
        # Wenn String: versuchen, als float zu interpretieren
        if isinstance(value, str):
            try:
                num = float(value)      # funktioniert für '1' und '1.1'
                if num.is_integer():    # prüfen ob es eigentlich eine ganze Zahl ist
                    return int(num)
                return num
            except ValueError:
                return None  # String war keine Zahl (z. B. 'hallo')

        # Falls anderer Typ → None
        return None

    def set_zaehler(self, zaehler_param):
        param = Fraction.parse_number(zaehler_param)
        if param is None:
            self.__zaehler = 0
        else: 
            if isinstance(param, float):
                self.__zaehler = int(round(param, 0))
            else:
                self.__zaehler = int(param)
    ...
    def nenner(self, nenner_param):
        param = Fraction.parse_number(nenner_param)
        if param is None:
            self.__nenner = 1
        else:
            if isinstance(param, float):
                param = int(round(param, 0))
            else:
                param = int(param)
